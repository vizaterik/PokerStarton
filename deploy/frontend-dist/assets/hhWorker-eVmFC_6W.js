...
function normalizeExternalHandId(raw) {
...
function contentFingerprint(h) {
  const net = h.hero_net == null || !Number.isFinite(h.hero_net) ? "" : String(Math.round(h.hero_net * 100) / 100);
  return [
    h.played_at || "",
    h.hero_hand_code || "",
    net,
...
function preferHandRow(a, b) {
  var _a, _b;
  const al = ((_a = a.raw_text) == null ? void 0 : _a.length) || 0;
  const bl = ((_b = b.raw_text) == null ? void 0 : _b.length) || 0;
  if (al !== bl) return al > bl ? a : b;
  return a.key <= b.key ? a : b;
}
function hasStrongFingerprint(h) {
...
const DAILY_HAND_UPLOAD_LIMIT = 5e3;
function handCalendarDay(playedAt) {
...
async function loadDayHandCounts(strategyId) {
...
async function insertHandBatch(strategyId, sessionId, hands, dayCounts, seenInImport) {
  const db = await openLocalDb();
  let inserted = 0;
  let duplicates = 0;
  let limitSkipped = 0;
  const counts = dayCounts ?? await loadDayHandCounts(strategyId);
  const seen = seenInImport ?? /* @__PURE__ */ new Set();
...
async function dedupeStrategyHands(strategyId) {
    ...
    if (rows.length === 1) {
      ...
      if (nid && (r.key !== want || r.external_hand_id !== nid)) {
        ...
        await new Promise((resolve, reject) => {
          const tx = db2.transaction([STORE_HANDS], "readwrite");
          const store = tx.objectStore(STORE_HANDS);
          store.delete(r.key);
  ...
  for (const next of keep.values()) {
    ...
    if (!old || old.external_hand_id !== next.external_hand_id) {
      toPut.push(next);
      for (const r of rows) {
        if (r.key !== next.key && normalizeExternalHandId(r.external_hand_id) === next.external_hand_id) {
...
async function listHandsForStrategy(strategyId, opts) {
  const db = await openLocalDb();
...
async function runImport(requestId, strategyId, files) {
  ...
  const seenInImport = /* @__PURE__ */ new Set();
  let totalEstimate = 0;
